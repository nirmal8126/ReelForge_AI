...
export function RetryButton({ challengeId }: RetryButtonProps) {
  ...
  async function onRetry() {
    setIsRetrying(true)
    try {
      const res = await fetch(`/api/challenges/${challengeId}/retry`, { method: 'POST' })
      if (!res.ok) {
        const data = await res.json().catch(() => null)
        throw new Error(data?.error || 'Failed to retry')
      }
      toast.success('Retrying challenge generation...')
      router.refresh()
    } catch (err) {
