...
interface DeleteLongFormButtonProps {
  jobId: string
  isProcessing: boolean
}

export function DeleteLongFormButton({ jobId, isProcessing }: DeleteLongFormButtonProps) {
  ...
  async function onDelete() {
    const confirmed = await confirmAction({
      title: 'Delete Long-Form Video?',
      text: 'This video and all its segments will be permanently deleted. This action cannot be undone.',
      confirmText: 'Delete',
      type: 'danger',
    })
    if (!confirmed) return
    ...
    try {
      const res = await fetch(`/api/long-form/${jobId}`, { method: 'DELETE' })
      if (!res.ok) {
